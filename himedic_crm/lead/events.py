...
def validate_lead(doc, method=None):
    ...
    if doc.phone and not doc.flags.get("ignore_dup"):
        ...
        if existing:
            # Defer the warning to after_insert — add_comment during validate would
            # reference a lead row that does not exist yet (BR-L-002).
            doc.flags.dup_warning = existing[0].name


def after_insert_lead(doc, method=None):
    """Auto-assign + push notification + apply scoring rules."""
    if doc.flags.get("dup_warning"):
        doc.add_comment("Comment", f"⚠ Trùng SĐT với lead {doc.flags.dup_warning} trong 30 ngày")
