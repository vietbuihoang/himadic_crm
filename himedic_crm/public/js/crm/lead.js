...
export const list = async () => {
  ...
  const head = screenHeader('Danh sách Khách tiềm năng',
    ...
    `${disabledBtn('Nhập từ Excel','px-3 py-1.5 text-sm bg-white border border-slate-200 rounded-lg')}
  ...
  const chips = [`Tất cả (${s.total||0})`,'Marketing chuyển','Sales tự phát triển','Quá SLA'];
  return head + `
    <div class="p-6">
      <div class="flex gap-2 mb-4 flex-wrap">
        ${chips.map((t,i)=>disabledBtn(t,`px-3 py-1.5 text-sm rounded-lg border ${i===0?'bg-brand-50 border-brand-300 text-brand-700':'bg-white border-slate-200 text-slate-600'}`)).join('')}
      </div>
      <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-slate-100">
            ${rows.map(l=>`
              <tr class="hover:bg-slate-50 cursor-pointer" onclick="window.__select('lead','detail')">
...
export const kanban = async () => {
  ...
  const head = screenHeader('Luồng chuyển đổi khách – Kanban', 'Bản chỉ đọc · dữ liệu trực tiếp từ hệ thống', '');
  return head + `
    <div class="p-6">
        ...
        ${cols.map(c=>`
          <div class="w-72 flex-shrink-0">
            ...
            <div class="bg-slate-50 rounded-xl p-2 min-h-[400px] space-y-2 border border-slate-200/60">
              ${(c.cards||[]).map(l=>`
                <div class="kanban-card bg-white rounded-lg p-3 border border-slate-200 cursor-pointer" onclick="window.__select('lead','detail')">
...
export const detail = async () => {
  let d;
  try {
    const { rows } = await api('lead', 'list', { limit: 1 });
    d = rows && rows[0] ? await api('lead', 'detail', { name: rows[0].name }) : null;
  ...
  const head = screenHeader(`Khách tiềm năng · ${d.lead_name||''}`,
    `Nguồn ${d.source||'—'} · Chiến dịch: ${d.campaign||'—'} · Cập nhật ${dfmt(d.modified)}`,
    `${disabledBtn('Đánh dấu Đã hủy','px-3 py-1.5 text-sm bg-white border border-slate-200 rounded-lg')}
     ${disabledBtn('Chuyển đổi → Tạo cơ hội','px-3 py-1.5 text-sm bg-emerald-600 text-white rounded-lg')}`);
  ...
  return head + `
    <div class="p-6 grid grid-cols-3 gap-6">
            ...
            ${acts.length ? acts.map(a=>`
              <div class="flex gap-3">
                ...
                <div class="flex-1"><div class="flex items-center justify-between">
                  <div><b>${a.activity_type||'Hoạt động'}</b> — ${a.subject||''}</div>
                  <span class="text-xs text-slate-400">${dfmt(a.activity_time||a.creation)}</span></div></div>
      ...
      <div class="space-y-4">
        ...
        ${d.interest_group?`<div class="bg-white rounded-xl border border-slate-200 p-4 text-sm">
          <div class="font-semibold mb-2">Quan tâm</div>
          <div class="flex flex-wrap gap-1">${tag(d.interest_group,'cyan')}</div></div>`:''}
      </div>
    </div>`;
};
